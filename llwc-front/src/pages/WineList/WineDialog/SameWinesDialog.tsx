...
const SameWinesDialog = (props: SameWinesDialogProps) => {
    ...
    const search = async () => {
        ...
        const query: FindSameWinesQuery = { show_drunk: true, show_stock: false };
                ...
                <FormControlLabel
                    ...
                    control={
                        <Switch
                            ...
                            onChange={event => {
                                setSearchKeys(prev => {
                                    return { ...prev, producer: event.target.checked };
                                });
                            }}
                        />
                    }
                    ...
                    {sameWines.map(wine => {
                        return (
                            <Paper elevation={3} key={wine.id} sx={{ m: 1, mb: 2 }}>
                                ...
                                {wine.tag_texts.length > 0 && <Typography>tag_texts: {wine.tag_texts.join(', ')}</Typography>}
                                ...
                                {wine.cepages.length > 0 && <Typography>cepages: {wine.cepages.map(cepage => cepage.abbreviation).join(', ')}</Typography>}
